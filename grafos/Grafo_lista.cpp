...
void Grafo_lista::carrega_grafo(const string &arquivo)
{
  ...
  //    [numNos] [dir=0/1] [vPond=0/1] [aPond=0/1]
  int dir, vPond, aPond;
  ...
  // 2) Se os vértices são ponderados, lê uma linha com 'numNos' pesos
...
bool Grafo_lista::eh_bipartido()
{
  ...
  // IDs de 1..numNos consecutivos Percorremos de 0 até (2^numNos -
...
int Grafo_lista::n_conexo()
{
  ...
  bool *visitado = new bool[numNos + 1]; // Considera que o primeiro vértice é 1
...
// Função dfs_articulacao que realiza a busca em profundidade e encontra as articulações
...
// Implementação da função que verifica se o grafo possui pontes
...
void Grafo_lista::insereVertice(int id, double pesoVertice = 1.0) {
  ...
  Grafo_lista::NoLista* Grafo_lista::getVizinhosIgnorandoDirecao(int idVert) {
    ...
    if (vOrigem != nullptr) {
        // Adiciona os vizinhos a partir das arestas de saída
        NoAresta* aAtual = vOrigem->getPrimeiraAresta();
        while (aAtual != nullptr) {
    ...
    // Se o grafo for direcionado, adiciona também as arestas de entrada
    if (this->direcionado) {
        NoVertice* vAtual = primeiroVertice;
        while (vAtual != nullptr) {
            NoAresta* aAtual = vAtual->getPrimeiraAresta();
