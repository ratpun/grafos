 ...
 * 
 * Este construtor inicializa um grafo representado por uma matriz de adjacência.
 * 
 * @param n O número de vértices no grafo. Deve ser maior que 0.
 * @param dir Um booleano indicando se o grafo é direcionado (true) ou não (false).
 * @param pondArestas Um booleano indicando se as arestas são ponderadas (true) ou não (false).
 * @param pondVertices Um booleano indicando se os vértices são ponderados (true) ou não (false).
 * 
 * @note Se o número de vértices (n) for menor ou igual a 0, uma mensagem de erro é impressa e o construtor retorna sem inicializar o grafo.
 */
 ...
 * 
 * @details Esta função insere um vértice no grafo, atribuindo um peso ao vértice
 ...
 * 
 * Esta função adiciona uma aresta entre dois nós no grafo, com um peso especificado.
 * Se o grafo não for direcionado, a aresta é adicionada em ambas as direções.
 * 
 ...
 * @param origem O índice do nó de origem.
 * @param destino O índice do nó de destino.
 * @return true se existir uma aresta entre os nós de origem e destino, false caso contrário.
 */
 ...
 *
 * Esta função lê um grafo de um arquivo especificado e inicializa a matriz de adjacência do grafo.
 * O arquivo deve conter a seguinte estrutura:
 * - Primeira linha: número de nós, indicador de direcionamento (1 para direcionado, 0 para não direcionado),
 *   indicador de vértices ponderados (1 para ponderado, 0 para não ponderado), indicador de arestas ponderadas
 *   (1 para ponderado, 0 para não ponderado).
 * - Se os vértices forem ponderados, as próximas linhas conterão os pesos dos vértices.
 * - As linhas seguintes conterão as arestas no formato: origem destino [peso] (o peso é opcional e só é lido se as arestas forem ponderadas).
 *
...
/**
 * @brief Cria um novo grafo a partir de um arquivo de descrição e salva em um arquivo de saída.
 *
 * Esta função lê a descrição de um grafo a partir de um arquivo de entrada, inicializa a matriz de adjacência
 * do grafo e, se especificado, cria um grafo completo. Em seguida, salva a representação do grafo em um arquivo
 * de saída.
 *
...
/**
 * @brief Verifica se o grafo representado pela matriz de adjacência é bipartido.
 * 
 * Um grafo é bipartido se os seus vértices puderem ser divididos em dois conjuntos disjuntos
 * tais que não haja arestas entre vértices do mesmo conjunto.
 * 
 ...
 *
 * Esta função verifica se o grafo representado pela matriz de adjacência é uma árvore.
 * Um grafo é considerado uma árvore se for conexo e tiver exatamente (número de nós - 1) arestas.
 *
 ...
 *
 * Esta função utiliza a Busca em Profundidade (DFS) para percorrer o grafo e contar o número de componentes conexas.
 * Ela inicializa um array booleano para rastrear os nós visitados, itera por cada nó e realiza uma DFS
 * se o nó não tiver sido visitado. Cada travessia DFS corresponde a uma nova componente conexa.
 *
 ...
 * @param visitado Um array booleano que mantém o controle dos vértices visitados.
 */
 ...
 *
 * Um grafo é considerado completo se houver uma aresta entre cada par de nós distintos.
 *
 ...
 * 
 * Uma ponte (ou aresta de corte) é uma aresta em um grafo cuja remoção aumenta o número de componentes conexas.
 * Esta função itera por todas as arestas na matriz de adjacência, remove temporariamente cada aresta e verifica
 * se o grafo se torna desconexo. Se a remoção de qualquer aresta causar a desconexão do grafo, a função retorna
 * true, indicando a presença de uma ponte.
 * 
